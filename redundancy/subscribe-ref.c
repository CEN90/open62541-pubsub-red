...
printServerVars(UA_Server *server) {
    ...
    if (UA_Variant_hasScalarType(&value, &UA_TYPES[UA_TYPES_INT64])) {
        int64_t int64Val = *(int64_t *)value.data;
        UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND, "SeqNr: %" PRId64, int64Val);
    }
}

static void
callbackPrintVars(UA_Server *server, void *data) {
    printServerVars(server);
}
...
run(UA_String *transportProfile, UA_NetworkAddressUrlDataType *networkAddressUrl) {
    ...
    UA_Server_addRepeatedCallback(server, callbackPrintVars, NULL, 1000, NULL);
